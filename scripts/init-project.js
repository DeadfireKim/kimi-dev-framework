...
- \`/pdca do {feature}\` - 실행
- \`/pdca check {feature}\` - 점검
- \`/pdca act {feature}\` - 개선
- \`/dev review\` - 코드 리뷰
- \`/dev retro\` - 회고
- \`/agile story {title}\` - 스토리 생성
...
function suggestNextSteps(tier) {
  ...
  console.log(`\n📋 Suggested next steps for ${tier} tier:`);
...
function main() {
  ...
  console.log(`\n🚀 Initializing KDF Project: ${projectName}`);
  console.log(`   Tier: ${tier.toUpperCase()}`);
  console.log('');

  // Check if already initialized
  if (fs.existsSync(KDF_DIR)) {
    console.log(`⚠️  KDF already initialized in this directory.`);
  ...
  console.log(`\n💡 Tip: Run "/pdca status" to check project status anytime.`);
}
